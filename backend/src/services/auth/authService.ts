...
const novu = new Novu('bb36211728c7555108b791dd293f51b5');
...
class AuthService {
  ...
  static async signup(
    firstName, lastName, businessName, country, phoneNumber, 
    email,
    password,
    invitationToken,
    tenantId,
    ...
    try {
      ...
      // invitation, creation of default tenant,
      // or default joining the current tenant
      await this.handleOnboardTenent(
        newUser,{
          firstName, 
          lastName, 
          name:businessName, 
  ...
  static async handleOnboardTenent(
    currentUser,
    tenantData,
    options,
  ) {

    const Roles = Permissions.asArray.reduce((acc, permission) => {
    ...
    const permissions = {
      ...
      tenant:'7eb00e1b-cf61-42d8-969f-095f1a1bfc47'
    }

   const tenant:any =  await new TenantService({
      ...options,
      currentUser,
    }).OnboardNewTenant( tenantData, options.transaction);

   
     await TenantUserRepository.createNewRoles(
      permissions,
      currentUser.id,
      tenant?.id,
      {
  ...
  static async signin(
    ...
    try {
      ...
      const novuresponse = await novu.trigger('generator', {
        to: {
          subscriberId: 'on-boarding-subscriber-id-123'
        },
        payload: {
