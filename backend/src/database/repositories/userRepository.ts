...
export default class UserRepository {
  ...
  static async updateCurrentTenant(
    data,
    options: IRepositoryOptions,
  ) {
    const currentUser = SequelizeRepository.getCurrentUser(
    ...
    const user = await options.database.user.findByPk(currentUser.id, {
      transaction,
    });
    const currentTenant = await options.database.tenantUser.findOne({
      where:{
        tenantId:data.id,
        userId: user.id,
        status: 'active'
  ...
  static async _fillWithRelationsAndFiles(
    ...
    output.inRoles = output.tenants[0]?.inRoles.map((role)=>({id:role?.id, roles:role.roles})) || []
    const mergedPermissions =  output.tenants[0]?.inRoles.reduce((acc, role) => {
      return lodash.mergeWith(acc, role.permissions, (objValue, srcValue) => {
        if (lodash.isBoolean(objValue) && lodash.isBoolean(srcValue)) {
          return objValue || srcValue; // Use OR to merge boolean values
        }
      });
    }, {});
    output.permissions = lodash.pickBy(mergedPermissions, value => value === true);
    output.tenantId = output.tenants[0]?.id
  ...
  static _mapUserForTenant(user, tenant) {
    ...
    const inRoles = tenantUser ? tenantUser.inRoles : [];
    user.inRoles = inRoles;
