...
import exporterFields from 'src/modules/store/list/storeListExporterFields';
...
const storeListActions = {
  ...
  doChangeSelected(payload) {
    return {
      type: storeListActions.SELECTEDS_CHANGED,
      payload,
    };
  },
  ...
  doExport: () => async (dispatch, getState) => {
    try {
      ...
      dispatch({
        type: storeListActions.EXPORT_STARTED,
      });
      ...
      new Exporter(
        exporterFields,
        i18n('entities.store.exporterFileName'),
      ).transformAndExportAsExcelFile(response.rows);

      dispatch({
        type: storeListActions.EXPORT_SUCCESS,
      });
    } catch (error) {
      Errors.handle(error);

      dispatch({
        type: storeListActions.EXPORT_ERROR,
      });
    }
  },
  ...
  doFetchCurrentFilter: () => async (
    ...
    dispatch(storeListActions.doFetch(filter, rawFilter, true));
  },
