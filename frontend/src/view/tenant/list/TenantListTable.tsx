...
const TenantListTable = (props) => {
  ...
  const hasTenantToEdit = useSelector(
    tenantSelectors.selectTenantToEdit,
  );
  const hasTenantToDestroy = useSelector(
    tenantSelectors.selectTenantToDestroy,
  );
        ...
        if (invitationToken(record)) {
          ...
          return (
            <div className="table-actions">
              ...
              {hasTenantToEdit(record) && (
                <Link to={`/tenant/${record.id}/edit`}>
                  {i18n('common.edit')}
                </Link>
              )}
              {hasTenantToDestroy(record) && (
                <Popconfirm
                  title={i18n('common.areYouSure')}
