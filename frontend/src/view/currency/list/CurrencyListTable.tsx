...
const CurrencyListTable = (props) => {
 const dispatch = useAppDispatch();
  ...
  const columns = [
      {
        title: i18n('entities.currency.fields.currency'),
        sorter: true,
        dataIndex: 'currency',
      },
      {
        title: i18n('entities.currency.fields.currencyIso'),
        sorter: true,
        dataIndex: 'currencyIso',
      },
    {
      title: '',
      dataIndex: '',
      ...
      render: (_, record) => (
        <div className="table-actions">
          <Link to={`/currency/${record.id}`}>
            {i18n('common.view')}
          </Link>
          {hasPermissionToEdit && (
            <Link to={`/currency/${record.id}/edit`}>
              {i18n('common.edit')}
            </Link>
          )}
