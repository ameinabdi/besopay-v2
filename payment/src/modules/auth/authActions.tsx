...
const authActions = {
  ...
  TWOFA_VERIFY_START: `${prefix}_TWOFA_VERIFY_START`,
  TWOFA_VERIFY_SUCCESS: `${prefix}_TWOFA_VERIFY_SUCCESS`,
  TWOFA_VERIFY_ERROR: `${prefix}_TWOFA_VERIFY_ERROR`,

  RESEND_TWOFA_VERIFY_START: `${prefix}_RESEND_TWOFA_VERIFY_START`,
  RESEND_TWOFA_VERIFY_SUCCESS: `${prefix}_RESEND_TWOFA_VERIFY_SUCCESS`,
  RESEND_TWOFA_VERIFY_ERROR: `${prefix}_RESEND_TWOFA_VERIFY_ERROR`,
  ...
  doRegisterEmailAndPassword: (firstName, lastName, businessName, phoneNumber, country, email, password )  => async (
    dispatch,
  ) => {
    try {
      ...
      const token = await service.registerWithEmailAndPassword(
        firstName, lastName,  businessName, phoneNumber, country,
        email,
        password,
  ...
  doVerifyEmail: (token) => async (dispatch, getState) => {
    try {
      const isLoading = authSelectors.selectLoadingVerifyEmail(
        getState(),
      );
  ...
  doSendTwoFAConfirmation: (token) => async (dispatch, getState) => {
    try {
      const isLoading = authSelectors.selectLoadingVerifyEmail(
        getState(),
      );
      ...
      dispatch({
        type: authActions.TWOFA_VERIFY_START,
      });

      await service.verifyTwoFA(token);

      await dispatch(authActions.doRefreshCurrentUser());

      dispatch({
        type: authActions.TWOFA_VERIFY_SUCCESS,
      });
    ...
    getHistory().push("/");
      ...
      Errors.showMessage(error);
      dispatch({
        type: authActions.TWOFA_VERIFY_ERROR,
      });
    getHistory().push("/");
    }
  },

  doResendTwoFAVerification: () => async (
    dispatch,
  ) => {
    try {
      dispatch({
        type: authActions.RESEND_TWOFA_VERIFY_START,
      });

      await service.resendTwoFA();

      Message.success(i18n('auth.passwordResetSuccess'));
      dispatch({
        type: authActions.RESEND_TWOFA_VERIFY_SUCCESS,
      });
    getHistory().push("/");
    } catch (error) {
      Errors.handle(error);
      ...
      dispatch({
        type: authActions.RESEND_TWOFA_VERIFY_ERROR,
      });
